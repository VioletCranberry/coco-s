...
must_haves:
  truths:
    ...
    - "Embeddings enable semantic search on code chunks"
    - "Chunks are stored in PostgreSQL with vector index"
    - "Different codebases can be indexed separately without conflicts"
  artifacts:
...
5. Verify semantic chunking (chunks align with function/class boundaries):
   ```bash
   docker exec -it coco-s-db-1 psql -U cocoindex -d cocoindex -c "SELECT filename, location FROM integration_test_chunks ORDER BY filename, location;"
   ```
   Locations should show chunks starting at function definitions (greet) and class definitions (Calculator), not arbitrary byte positions.
</verification>
