...
must_haves:
  ...
  key_links:
    ...
    - from: "src/cocosearch/indexer/flow.py"
      ...
      via: "ensure_symbol_columns call in flow setup"
      pattern: "ensure_symbol_columns"
---
...
1. Import the symbol extraction function AND schema migration:
```python
...
2. Call `ensure_symbol_columns()` BEFORE indexing begins, in the flow setup section (after index spec is created, before chunk processing). This ensures columns exist before any data is inserted:
```python
# Ensure symbol columns exist in target table
# This must happen before CocoIndex begins inserting data
ensure_symbol_columns(conn, f"{index_name}_chunks")
```

Note: The connection (`conn`) should be obtained from the existing database connection used in flow.py. If flow.py doesn't directly have a connection, add this call to the index creation entry point (e.g., in CLI or wherever the flow is invoked) before calling the flow.

3. In the chunk processing loop (inside `with file["chunks"].row() as chunk:`), add symbol extraction AFTER existing metadata extraction:
```python
...
4. Update `code_embeddings.collect()` to include symbol fields:
```python
...
Should succeed without errors.
  ...
  <done>Indexing flow extracts and stores symbol metadata for each chunk, with schema migration called before indexing begins</done>
</task>
...
Add to existing test file, following the established patterns. Use @pytest.mark.unit decorator.
  ...
  <verify>
Run symbol-specific tests first, then full test suite:
```bash
uv run pytest tests/unit/indexer/test_flow.py -v -k symbol
uv run pytest tests/unit/indexer/test_flow.py -v
```
Both commands should pass - symbol tests verify new functionality, full suite verifies no regressions.
  </verify>
  <done>Flow unit tests verify symbol extraction integration and full flow works</done>
</task>
