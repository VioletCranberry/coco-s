...
must_haves:
  truths:
    ...
    - "_detect_project falls through to cwd as last resort, returning (Path.cwd(), 'cwd') -- never returns None"
...
**2. `async def _detect_project(ctx: Context) -> tuple[Path, str]`**

IMPORTANT: The return type is `tuple[Path, str]` -- NOT `tuple[Path | None, str]`. This function ALWAYS returns a valid Path because step 4 (cwd) is an unconditional fallback. It never returns None.
...
- Return `(Path.cwd(), "cwd")`
- This ALWAYS executes if steps 1-3 don't return. There is no None return path.
...
- `_detect_project` is async, follows the 4-step priority chain, and always returns a valid Path (never None)
...
6. `python -c "import inspect; from cocosearch.mcp.project_detection import _detect_project; hints = inspect.get_annotations(_detect_project); print(hints)"` -- verify return type annotation does NOT include None
</verification>
...
- _detect_project is async, implements the full priority chain (roots > query_param > env > cwd), and always returns a valid Path (never None)
