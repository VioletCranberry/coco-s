...
must_haves:
  truths:
    ...
    - "Auto-detect tests work with the new _detect_project-based detection"
  ...
  artifacts:
    ...
    - path: "tests/unit/mcp/test_server_autodetect.py"
      provides: "Updated auto-detect tests using _detect_project mocking"
  key_links:
...
<objective>
Create comprehensive tests for the project detection module, update existing MCP server tests for async tool signatures, and fix the two pre-existing test failures in TestIndexCodebase.
...
Output: Full test coverage for phase 45 changes, all MCP tests passing.
</objective>
...
<task type="auto">
  <name>Task 2: Update existing MCP tests for async + fix pre-existing failures</name>
  <files>tests/unit/mcp/test_server.py, tests/unit/mcp/test_server_autodetect.py</files>
  <action>
...
**Part B: Update test_server.py for async search_code**
...
For each of these tests, search_code now requires `ctx: Context`. Create a simple mock_ctx fixture or helper. Since these tests pass `index_name` explicitly, `_detect_project` is never called (the `if index_name is None` block is skipped). However, FastMCP's `@mcp.tool()` decorator handles Context injection at the framework level -- when calling the raw function directly in tests, you must pass `ctx` explicitly.
...
**Part C: Update test_server_autodetect.py for async + _detect_project**

All tests in this file call `search_code` and mock `find_project_root`. They need:
1. Convert to async (add `@pytest.mark.asyncio`, `async def`, `await`)
2. Pass `ctx=mock_ctx` to search_code
3. Replace `patch("cocosearch.mcp.server.find_project_root", ...)` with `patch("cocosearch.mcp.server._detect_project", ...)` using AsyncMock

The key mapping from old to new mocking:
- Old: `find_project_root` returns `(path, "git")` or `(None, None)`
- New: `_detect_project` returns `(path, "roots")` or `(None, "cwd")`

For each test class:

**TestSearchCodeAutoDetect:**
- `test_auto_detects_from_cwd`: Replace find_project_root mock with _detect_project AsyncMock returning `(mock_root, "roots")`. Also mock `find_project_root` (the local re-import inside search_code for walking up to git root). Add ctx parameter.
- `test_returns_error_when_no_project`: Mock _detect_project returning `(None, "cwd")`. Add ctx parameter.
- `test_returns_error_when_index_not_found`: Mock _detect_project returning `(mock_root, "roots")`. Mock find_project_root (local import). Add ctx parameter.
- `test_returns_collision_error`: Same pattern.
- `test_uses_explicit_index_name`: Add ctx, make async. _detect_project should NOT be called.
...
**TestAutoDetectErrorResponses:** These tests call `search_code` -- update all to async with ctx and _detect_project mocking.

Note on the local `find_project_root` import: In the updated server.py, search_code does `from cocosearch.management import find_project_root as find_root` inside the function body after _detect_project returns a path. Tests that exercise auto-detection need to mock this local import. The mock target is `cocosearch.mcp.server.find_project_root` if it's imported at the module level, or use `patch("cocosearch.management.context.find_project_root")`. Check the actual import pattern in the updated server.py from Plan 02 and mock accordingly.

**Part D: Update test_server_context.py for async search_code**

All tests in TestContextInResponse and TestContextParameterCombinations and TestContextEdgeCases call search_code directly. They all need:
...
- All new async test versions
- All project_detection tests
  </verify>
  <done>
- Pre-existing test failures fixed (register_index_path mock added)
- All search_code tests converted to async with ctx parameter
- All autodetect tests updated to mock _detect_project instead of find_project_root
- All context tests updated for async
- `python -m pytest tests/unit/mcp/ -v` passes with zero failures
  </done>
</task>
...
- All search_code tests use async/await with ctx parameter
- All autodetect tests mock _detect_project instead of find_project_root
