...
Output: Updated `formatter.py` with metadata in JSON and annotations in pretty output. Updated `server.py` with metadata in MCP response. Tests for all changes in separate test files.
</objective>
...
<task type="auto">
  ...
  2. **Update the `language` parameter description** in the `search_code` function signature:
     ...
     to:
     ```python
     Field(description="Filter by language (e.g., python, typescript, hcl, dockerfile, bash). Aliases: terraform=hcl, shell/sh=bash. Comma-separated for multiple.")
     ```
     This documents the new DevOps language support, aliases, and multi-language capability added in Plan 01.
  ...
  - Language parameter description updated to mention DevOps languages, aliases, and comma support
  - Metadata fields use empty strings when unavailable (consistent shape)
  </done>
</task>

<task type="auto">
  <name>Task 3: Add tests for formatter metadata and annotations</name>
  ...
  <verify>
  Run `cd /Users/fzhdanov/GIT/personal/coco-s && python -m pytest tests/search/test_formatter.py -v` -- all existing and new tests pass.
  </verify>
  <done>
  ...
  - All existing formatter tests still pass (no regressions)
  </done>
</task>

<task type="auto">
  <name>Task 4: Add tests for MCP metadata response</name>
  <files>tests/mcp/test_server.py</files>
  <action>
  1. **Update TestSearchCode** (or add new class TestSearchCodeMetadata):
  ...
  <verify>
  Run `cd /Users/fzhdanov/GIT/personal/coco-s && python -m pytest tests/mcp/test_server.py -v` -- all existing and new tests pass.
  Run `cd /Users/fzhdanov/GIT/personal/coco-s && python -m pytest tests/ -v` -- full test suite passes (no regressions).
  </verify>
  <done>
  - TestSearchCode (or TestSearchCodeMetadata) verifies metadata fields in MCP response
  - Empty metadata produces consistent shape with empty strings
  - All existing MCP tests still pass (no regressions from SearchResult changes)
...
- MCP language parameter description updated for DevOps languages and aliases
